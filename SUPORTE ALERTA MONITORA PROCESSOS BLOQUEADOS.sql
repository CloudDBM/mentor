...
CREATE OR ALTER proc [dbo].[spu_alerta_bloqueio] as
...
create OR ALTER procedure [dbo].[spu_checkblocking]
as
	...
	begin
		...
		if @spid <> @blocked 
			insert into [DBA_ELEVA_MANUT].[dbo].[blocktable] values (@spid,@blocked,@eventtype1,@eventtype2,@waittime,@hostname,getdate(),@program_name, @program_name2,@host_blocking)
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA - Alerta - Monitora Processos Bloqueados_1', 
		@enabled=1, 
...
exec spu_checkblocking', 
		@database_name=N'DBA_ELEVA_MANUT', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
